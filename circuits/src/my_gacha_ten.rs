...
pub const TEN_MODULUS_EXPONENT: u64 = 32;
pub const TEN_MODULUS: u64 = 1 << TEN_MODULUS_EXPONENT;
pub const TEN_MULTIPLIER: u64 = 1664525;
pub const TEN_INCREMENT: u64 = 1013904223;
pub const TEN_DRAWS: usize = 10;

#[derive(Clone, Debug)]
struct TenCell(AssignedCell<Fp, Fp>);

#[derive(Clone, Debug)]
pub struct MyTenGachaConfig {
    adv: [Column<Advice>; 3],
...
impl MyTenGachaConfig {
    pub fn configure(meta: &mut ConstraintSystem<Fp>) -> Self {
        ...
        meta.create_gate("my ten lcg", |meta| {
            ...
            let a = Expression::Constant(Fp::from(TEN_MULTIPLIER));
            let m = Expression::Constant(Fp::from(TEN_MODULUS));
            let c = Expression::Constant(Fp::from(TEN_INCREMENT));
        ...
        meta.create_gate("my ten rarity", |meta| {
    ...
    fn next_value(prev_val: Value<Fp>) -> Value<Fp> {
        prev_val.map(|a| a * Fp::from(TEN_MULTIPLIER) + Fp::from(TEN_INCREMENT))
    }
    ...
    fn assign_first_row(
        ...
        seed: Value<Fp>,
    ) -> Result<(TenCell, TenCell), Error> {
        layouter.assign_region(
            || "my ten lcg first row",
            |mut region| {
                let offset = 0;

                self.selector.enable(&mut region, offset)?;

                let seed_val = seed.map(rem_ten);
                let next_val = Self::next_value(seed_val);
                let rem_val = next_val.map(rem_ten);
                let quot_val = next_val.map(quot_ten);
    ...
    fn assign_next_row(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                prev.0
                    .copy_advice(|| "prev", &mut region, self.adv[0], offset)?;
                let next_cell = region
                    .assign_advice(|| "next value mod m", self.adv[1], offset, || rem_val)
                    .map(TenCell)?;
                region.assign_advice(|| "quotient", self.divisor, offset, || quot_val)?;
...
pub struct MyTenGachaCircuit {
    pub seed: Value<Fp>,
}

impl Circuit<Fp> for MyTenGachaCircuit {
    type Config = MyTenGachaConfig;
    ...
    fn configure(meta: &mut ConstraintSystem<Fp>) -> Self::Config {
        MyTenGachaConfig::configure(meta)
    }
    ...
    fn synthesize(
        ...
        let (mut prev, mut prev_rarity) =
            config.assign_first_row(layouter.namespace(|| "first row"), self.seed)?;
        config.expose_public(layouter.namespace(|| "out_0"), &prev, 0)?;
        config.expose_public(layouter.namespace(|| "rarity_0"), &prev_rarity, TEN_DRAWS)?;

        for i in 1..TEN_DRAWS {
            let (next_cell, rarity_cell) = config.assign_next_row(
                layouter.namespace(|| format!("next row {}", i)),
                &prev,
            )?;
            config.expose_public(
                layouter.namespace(|| format!("out_{}", i)),
                &next_cell,
                i,
            )?;
            config.expose_public(
                layouter.namespace(|| format!("rarity_{}", i)),
                &rarity_cell,
                TEN_DRAWS + i,
            )?;
            prev = next_cell;
...
fn rem_ten(input: Fp) -> Fp {
    let divisor: usize = (TEN_MODULUS_EXPONENT / 8).try_into().unwrap();
...
pub fn my_ten_get_randoms(seed: u64) -> [u64; TEN_DRAWS] {
    let mut ret = seed % TEN_MODULUS;
    let mut outs = [0u64; TEN_DRAWS];
    for i in 0..TEN_DRAWS {
        ret = (ret * TEN_MULTIPLIER + TEN_INCREMENT) % TEN_MODULUS;
        outs[i] = ret;
    }
    outs
}

pub fn my_ten_generate_setup_params(k: u32) -> Params<EqAffine> {
    Params::<EqAffine>::new(k)
}

pub fn my_ten_empty_circuit() -> MyTenGachaCircuit {
    MyTenGachaCircuit {
        seed: Value::unknown(),
    }
}

pub fn my_ten_create_circuit(seed: u64) -> MyTenGachaCircuit {
    MyTenGachaCircuit {
        seed: Value::known(Fp::from(seed)),
    }
}

pub fn my_ten_generate_keys(
    params: &Params<EqAffine>,
    circuit: &MyTenGachaCircuit,
...
mod tests {
    ...
    #[test]
    fn test_my_ten_randoms_consistency() {
        let seed: u64 = 987654321;
        let outs = my_ten_get_randoms(seed);

        let circuit = MyTenGachaCircuit {
            seed: Value::known(Fp::from(seed)),
        };

        let public_inputs: Vec<Fp> = outs.iter().map(|x| Fp::from(*x)).collect();
